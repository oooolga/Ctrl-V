...
class BBOXFrameAttention(nn.Module):
    def __init__(self, num_frames, in_channels=4, out_channels=4, num_layers=2, cross_attention_dim=None,
                       norm_num_groups=4, use_linear_projection=False, upcast_attention=False,
                       attention_type='default'):
        super().__init__()
        self.attn = Transformer2DModel(
            num_attention_heads=num_frames,
    ...
    def forward(self, image_latents, bbox_latents):
        return self.attn(image_latents, bbox_latents).sample
...
class Transformer2DModel(ModelMixin, ConfigMixin):
    ...
    def _get_output_for_continuous_inputs(self, hidden_states, residual, batch_size, height, width, inner_dim):
        ...
        output = hidden_states*self.rz_weight + residual.repeat(1, self.out_channels//self.in_channels, 1, 1)
        return output
